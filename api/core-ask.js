...
function* train_recognizer(skills) {
	...
	skills.map(skill => {
		if (skill.intent) {
			...
			intent.keywords.map(keyword => {
				console.log(strip(keyword),'-',skill.name);
	...
	if (failed.length > 0) {
		logger.warn(`${failed.length} queries were not routed correctly, attempting re-education.`);
	}
	for (let i = 0; i < failed.length; i++) {
		logger.warn(`Expected (${failed[i].expected}), used (${failed[i].skill}) for: '${failed[i].keyword}'`);
	}
	while (failed.length > 0) {
		if (retrainCount > MAX_RETRAINS) {
			logger.warn(`Maximum number of re-trainings reached with ${failed.length} failures.`);
	...
	if (failed.length == 0) {
		logger.debug(`Re-education successful after ${retrainCount + 1} iterations.`);
	}
}
...
function* classify(q, user, token, isResponse) {
	...
	if (hard_skills.length > 0) {
		if (hard_skills.length > 1) {
			logger.debug(`Multiple hard skills for query '${q}'`)
		...
		if (conditional_skills.length > 1) {
			logger.debug(`Multiple conditional skills for query '${q}'`)
...
function* query(input, user, token) {
	...
	logger.debug(`Using skill ${classification.skill.name} for ${query}`)
